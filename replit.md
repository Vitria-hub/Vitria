...
## Recent Security & Performance Enhancements (November 2025)

### OAuth Cookie Security
- Added conditional `Secure` flag based on protocol detection (HTTPS)
- Changed `SameSite` from `Lax` to `Strict` for CSRF protection
- **Known Limitation**: `HttpOnly` flag cannot be set because cookie is client-side (`document.cookie`). Full security requires refactoring to server-side cookie management.

### Admin Procedure Performance
- Eliminated redundant database query in `adminProcedure` by using cached `ctx.user.role` from `protectedProcedure` context
- Reduces latency by ~50-100ms per admin request

### TypeScript Type Safety
- Added explicit `AuthContext` type extending `Context` with properly typed `user` object
- Added `UserData` type with strict role enum
- Used `satisfies` clause for type narrowing in protectedProcedure
- Removed 4 instances of `any` types in agency router

### Known Technical Debt
- **Rate Limiting**: No rate limiting on public endpoints (2hrs effort)
- **File Validation**: Missing file type/size validation for uploads (2hrs effort)
- **'any' Types**: 16+ remaining instances across codebase (4hrs effort)
- **Test Coverage**: 0% test coverage (8hrs effort)
- **Console Logs**: 70+ production console.log statements
- **Supabase Types**: Manual Insert/Update type additions cause TypeScript inference collapse; requires CLI-generated types